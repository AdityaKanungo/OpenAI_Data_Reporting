...
def generate_plot_code(prompt, df):
    # Convert the DataFrame to a string in a CSV-like format for the prompt
    df_string = df.to_string()
    ...
    messages = [
    {
        ...
        Create a python code to {prompt} in streamlit using potly express.
        Assume the DataFrame 'df' is pre-loaded with the necessary data. Don't try to # Load the data directly write the code after that.
        convert the columns to their appropriate data types 
        don not include ``` in the start and end of python code
        Don't add unnecessary code, eg: st.set_page_config(layout="wide")

        Input data: {df}

        Instructions : Plot should contain {prompt}
    ...
    try:
        ...
        # Extract the code directly from the 'content' key of the 'message' dictionary
...
with st.container():
    ...
    if st.session_state.get('query_data_clicked', False) and result_query_prompt:
        with st.spinner("Processing..."):
            ...
            if analysis_type == "Plot a graph":
                python_code = generate_plot_code(result_query_prompt, df_h)
                if python_code:
                    st.code(python_code)
